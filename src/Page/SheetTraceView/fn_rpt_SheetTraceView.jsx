...
function fn_rpt_SheetTraceView() {
  ...
  const [lblShtMachine, setlblShtMachine] = useState({
    visible: "",
    value: "",
  });
  const [lblTitleMachine, setlblTitleMachine] = useState({
    visible: "",
    value: "",
  });
  ...
  const [hfMaterialN1, sethfMaterialN1] = useState(
    "http://10.17.100.236/Reports/report/Traceability%20Reports/N1/Valor/PcbTraceReference?PcbID=#SHEET_NO#"
  );
  ...
  const [btnReflow, setbtnReflow] = useState({
    disbled: true,
    value: "",
    style: {},
  });
  const [btnXRay, setbtnXRay] = useState({
    disbled: true,
    value: "",
    style: {},
  });
  const [btnAOI, setbtnAOI] = useState({ disbled: true, value: "", style: {} });
  const [btnAOICOA, setbtnAOICOA] = useState({
    disbled: true,
    value: "",
    style: {},
  });
  const [btnSMTInt, setbtnSMTInt] = useState({
    disbled: true,
    value: "",
    style: {},
  });
  ...
  const ViewData = async (sheetno, dropdawn) => {
    ...
    try {
      ...
      if (dt && dt.length > 0) {
        if (dt[0].lss_lot_no !== null) {
          ...
          if (dt[0].lss_machine_no && dt[0].lss_machine_no.trim() !== "") {
            ...
            setlblShtMachine((prevState) => ({
              ...prevState,
              value: dt[0].lss_machine_no.trim(),
              visible: true,
            }));
          }
        }
      ...
      if (dt.length > 0) {
        ...
        if (
          StrResult.toUpperCase() == "NG" ||
          StrResult.toUpperCase() == "FAIL" ||
          StrResult.toUpperCase() == "BADMARK"
        ) {
          setbtnSPI((prevState) => ({
            ...prevState,
      ...
      if (dt.length > 0) {
        ...
        if (
          dt[0].xray_result == "GOOD" ||
          dt[0].xray_result == "OK" ||
          dt[0].xray_result == "PASS"
        ) {
          setbtnXRay((prevState) => ({
            ...prevState,
            value: dt[0].xray_result,
            style: { backgroundColor: "green" },
          }));
        } else {
          setbtnXRay((prevState) => ({
      ...
      if (dtSMPJ.length > 1) {
        ...
        setlblCavity((prevState) => ({
          ...prevState,
          value: "Connect Sht&Pcs",
        }));
      ...
      if (txtSheetNo.trim() !== "" && txtProduct.trim() !== "") {
          ...
          if (StrResult.trim() !== "") {
            ...
            if (
              ...
              setbtnAOIEFPC((prevState) => ({
                ...prevState,
                value: StrResult,
                style: { backgroundColor: "red", disabled: false },
              }));
            }
          }
        }
        ...
        if (dtReflow.length > 0) {
          setbtnReflow((prevState) => ({
            ...prevState,
            value: dtReflow[0].REFLOW_RESULT,
          }));
          if (btnReflow == "NG") {
            setbtnReflow((prevState) => ({
              ...prevState,
        ...
        if (dtData.length > 0) {
          ...
          if (
            ...
            for (let i = 0; i > dtData.length; i++) {
              ...
              if (
                StrResult.substring(0, 1).toUpperCase() === "E" ||
                StrResult.substring(0, 1).toUpperCase() === "W"
              ) {
                break;
              }
            }
        ...
        if (btnSPI.value == "") {
          ...
          if (btnAOI.trim() !== "") {
            ...
            if (dt.length > 0) {
              ...
              settxtAOIMachine(dt[0].aor_machine_no);
              switch (
                btnAOI.toUpperCase() // สมมุติว่า btnAOIText เป็นค่าของ btnAOI.Text
              ) {
                case "GOOD":
                case "OK":
                case "JUDGE":
